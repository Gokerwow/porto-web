...
export default function BlurText({ children } : BlurProps) {
    ...
    return (
        <motion.div initial className="absolute z-20 left-50 top-1/4">
            <div onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>{children.split("").map((l, i) => {
                return <h1 key={i} className={`text-8xl p-7 text-right w-[600px]  ${Transcity.className}`}>{children}</h1>
            })}</div>
        </motion.div>
    )
}