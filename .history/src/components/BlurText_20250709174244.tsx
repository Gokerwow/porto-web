    ...
    return (
        <motion.div initial="initial" animate="animate" className="absolute z-20 left-50 top-1/4">
            <div onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} className='w-[500px] text-right'>{children.split("").map((l, i) => {
                return <motion.span variants={{ initial: { x:"-100%", opacity:0, filter: "blur(10px)" }, animate: { x:0, opacity:1, filter: "blur(0px)" } }} transition={{ type:"tween", ease:"backOut", delay: 0.25 * i }} key={i} className={`text-8xl text-right ${Transcity.className}`}>{l}</motion.span>
            })}
            </div>
        </motion.div>
    )
