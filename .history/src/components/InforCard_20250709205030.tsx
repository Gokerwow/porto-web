...
const InfoCard = forwardRef<HTMLDivElement, InfoCardProps>(({
    icon, title, imageUrl, imageAlt, description, subDescription,
}, ref) => {
    const { icon, title, imageUrl, imageAlt, description, subDescription } = props;

    return (
        <div ref={ref} className="flex-1 flex flex-col items-center p-8 bg-slate-50 rounded-xl shadow-md hover:shadow-lg hover:-translate-y-1 transition-all duration-300 min-w-[300px] border border-slate-200">
