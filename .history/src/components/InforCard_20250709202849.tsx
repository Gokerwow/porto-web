...
export default function InforCard({ icon, title, imageUrl, imageAlt, description, subDescription }: InfoCardProps) {
    const cardVariants = {
        hidden: { opacity: 0, y: 1150 },
        visible: { opacity: 1, y: 0 },
    } as const;

    return (
        <motion.div variants={cardVariants} className="flex-1 flex flex-col items-center p-8 bg-slate-50 rounded-xl shadow-md hover:shadow-lg hover:-translate-y-1 transition-all duration-300 min-w-[300px] border border-slate-200">
