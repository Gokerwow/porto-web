...
export default function Home() {
  ...
  const loadingDuration = 6; // Durasi dalam detik

  useEffect(() => {
    // Timer untuk menyembunyikan splash screen setelah 2.5 detik
    const timer = setTimeout(() => {
      setIsLoading(false);
      // Simpan status di session storage agar tidak muncul lagi saat refresh
      sessionStorage.setItem("hasLoadedBefore", "true");
    }, loadingDuration * 1000);

    // Cek jika pengguna sudah pernah memuat halaman di sesi ini
