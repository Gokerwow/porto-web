...
export default function ProjectCard({ title, description, media, tags, year, accent, index }: ProjectCardProps) {
    return (
        <div className="w-full h-[450px] bg-gray-300 inset-0 rounded-lg shadow-xl relative group transform-3d p-10">
            <div className="absolute inset-0 h-fit rounded-xl bg-white shadow-lg transform-3d translate-z-[75px] ">
                {/* Media container with gradient overlay */}
                <div className="relative aspect-video transform-3d ">
                    {media.endsWith('.mp4') ? (
                        <video
                            autoPlay loop muted playsInline
                            className="w-full h-full object-cover rounded-t-lg flex flex-col opacity-90 group-hover:opacity-100 transition-opacity"
                        >
                        ...
                        <Image
                            ...
                            alt={title}
                            fill
                            className="w-full h-full object-cover rounded-t-lg opacity-90 group-hover:opacity-100 transition-opacity"
                        />
                    )}
