...
export default function ProjectCard({ title, description, media, tags, year, accent, index }: ProjectCardProps) {
    return (
        <div className="w-full h-96 bg-gray-300 inset-0 rounded-lg shadow-xl relative group " style={{ transformStyle: "preserve-3d" }}>
            <div className="absolute inset-4 h-fit rounded-xl bg-white shadow-lg" style={{ transformStyle: "preserve-3d", transform: "translateZ(75px)" }}>
                {/* Media container with gradient overlay */}
                <div className="relative h-64 transform-3d">
                    {media.endsWith('.mp4') ? (
                        <video
                            autoPlay loop muted playsInline
                            className="w-full h-full object-cover aspect-video opacity-90 group-hover:opacity-100 transition-opacity"
                        >
