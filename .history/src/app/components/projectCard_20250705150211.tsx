...
export default function ProjectCard({ title, description, media, tags, year, accent, index }: ProjectCardProps) {
    return (
        // [1] Container "Panggung" (Scene). 
        // Hanya bertanggung jawab untuk perspektif. Elemen ini TIDAK berputar.
        <div className="group" style={{ perspective: '1000px' }}> 
            {/* Catatan: Tailwind `perspective-1000` adalah utility class. 
              Menggunakannya di sini kadang bisa konflik. Menggunakan inline style lebih aman untuk `perspective`.
              Atau, Anda bisa definisikan custom class di global.css.
            */}

            {/* [2] Kartu yang akan berputar. Ini adalah elemen yang memiliki `transform-style: preserve-3d`. */}
            <div
                key={index}
                className="transform-3d rotate-y-90 h-full w-full rounded-2xl transition-all duration-700 group-hover:transform group-hover:rotate-x-[3deg] group-hover:rotate-y-[6deg] group-hover:scale-[1.03]"
            >
