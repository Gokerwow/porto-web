...
export default function ProjectCard({ title, description, media, tags, year, accent, index }: ProjectCardProps) {
    ...
    const rotateX = useTransform(MouseYSpring, [0, 1], ['57.5deg', '-57.5deg']);
    const rotateY = useTransform(MouseXSpring, [0, 1], ['-57.5deg', '57.5deg']);
    ...
    return (
        <motion.div onMouseMove={HandleMouseMove} className="w-full h-[500px] bg-gray-300 inset-0 rounded-lg shadow-xl relative group transform-3d rotate-y-45" >
            <div className="absolute inset-4 rounded-xl bg-white shadow-lg transform-3d translate-z-[75px]">
                ...
                <div className="relative aspect-video transform-3d">
                    ...
                    <span className={`absolute top-4 right-4 px-3 py-1 bg-white/80 text-gray-800 text-sm font-medium rounded-full shadow-sm backdrop-blur-sm transform-3d translate-z-[35px]`}>
                        {year}
                    </span>
                </div>
                {/* Content */}
