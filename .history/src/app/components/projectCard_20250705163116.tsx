...
export default function ProjectCard({ title, description, media, tags, year, accent, index }: ProjectCardProps) {
    return (
        <div className="w-full h-[450px] bg-gray-300 inset-0 rounded-lg shadow-xl relative group transform-3d">
            <div className="absolute inset-0 h-full rounded-xl bg-white shadow-lg transform-3d translate-z-[75px] ">
                {/* Media container with gradient overlay */}
                <div className="relative aspect-video transform-3d ">
                    {media.endsWith('.mp4') ? (
                        <video
                            autoPlay loop muted playsInline
                            className="w-full h-full object-cover  flex flex-col opacity-90 group-hover:opacity-100 transition-opacity"
                        >
                ...
                {/* Content */}
                <div className="p-6 transform-3d translate-z-[25px] flex-1 overflow-y-auto">
                    ...
                    <div className="flex justify-between items-center">
                        <button className={`px-0 py-2 font-medium text-sm bg-gradient-to-r ${accent} bg-clip-text text-transparent group-hover:brightness-110 transition-all`}>
