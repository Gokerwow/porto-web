...
export default function ProjectCard({ title, description, media, tags, year, accent, index }: ProjectCardProps) {
    ...
    return (
        <Link href={`/projects/${index}`}>
            <motion.div onMouseLeave={HandleMouseLeave} onMouseMove={HandleMouseMove} className="w-full h-[500px] bg-gray-300 inset-0 rounded-lg shadow-xl relative group transform-3d " style={{ rotateX, rotateY }} >
                <div className="absolute inset-4 rounded-xl bg-white shadow-lg transform-3d translate-z-[75px]">
                    ...
                    <div className="p-6 transform-3d translate-z-[50px]">
                        ...
                        <div className="flex justify-between items-center">
                            <Link href={`/projects/${index}`} className={`relative z-10 px-0 py-1 font-medium text-sm bg-gradient-to-r ${accent} bg-clip-text text-transparent group-hover:brightness-110 transition-all cursor-pointer`}>
